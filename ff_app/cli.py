...
def summarize_week(league, week):
    ...
    for box in box_scores:
        # box.home_team and box.away_team are Team objects, not IDs
        home_name = box.home_team.team_name if box.home_team else 'BYE'
        away_name = box.away_team.team_name if box.away_team else 'BYE'
        lines.append(
            f"{away_name} {box.away_score} @ {home_name} {box.home_score} "
            f"(proj: {box.away_projected:.1f}-{box.home_projected:.1f})"
        )
    return "\n".join(lines)
...
def lineup_tips(league, week):
    ...
    for box in scores:
        ...
        for team, lineup in [
            ...
            playable = [
                p for p in lineup
                if p.slot_position not in ('BE', 'IR', 'FA')
            ]
            ...
            for starter in playable:
                better = [
                    b for b in bench
                    if b.position == starter.position
                    and b.projected_points > starter.projected_points + 1.0
                ]
                if better:
                    best = sorted(
                        better,
                    ...
                    delta = round(
                        best.projected_points - starter.projected_points, 2
                    )
                    tips.append(
                        f"{team.team_name}: Consider starting {best.name} "
                        f"over {starter.name} (+{delta} proj)"
                    )
...
def free_agent_suggestions(league, week, size):
    ...
    sorted_agents = sorted(
        agents,
        key=lambda p: (p.projected_points, p.percent_owned),
        reverse=True
    )[:10]
    lines = [
        f"{p.name} ({p.position}, {p.proTeam}) proj {p.projected_points:.1f}, "
        f"owned {p.percent_owned}%"
...
def main(argv=None):
    ...
    for rank, team in enumerate(league.power_rankings(week), start=1):
        wins = sum(1 for o in team.team.outcomes[:week] if o == 'W')
        print(f"{rank}. {team.team.team_name} (wins: {wins})")
    print("\n=== Lineup Tips ===")
